 ...
 * Simple example:
 * <code>
 * $this->load->library('yubiAuth');
 * $otp = "ccbbddeertkrctjkkcglfndnlihhnvekchkcctif";
 *
 * # Generate a new id+key from https://api.yubico.com/get-api-key/
 * $this->yubiauth->init('42', 'FOOBAR=');
 * $this->yubiauth->addURLPart('10.8.0.1:9081/wsapi/verify');
 * $auth = $this->yubiauth->verify($otp);
 * if ($this->yubiauth->isError) {
 *    print "<p>Authentication failed: " . $this->yubiauth->getLastMessage();
 *    print "<p>Debug output from server: " . $this->yubiauth->getLastResponse();
 * } else {
...
class YubiAuth {
    ...
    **/
     ...
     * @access public
    **/
    function init($id, $key = '', $https = 0, $httpsverify = 1){
        $this->_id =  $id;
    ...
    **/
    function verify($token, $use_timestamp=null, $wait_for_all=False, $sl=null, $timeout=null){
        ...
        while($URLpart=$this->getNextURLpart()){
            ...
            curl_setopt($handle, CURLOPT_USERAGENT, "YubiAuth-PHP");
                ...
                if (!$wait_for_all && ($valid || $replay)) 
                  {
                    ...
                    curl_multi_close($mh);
                    if ($replay) return $this->raiseError('REPLAYED_OTP');
